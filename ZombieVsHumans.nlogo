...
turtles-own []
zombies-own [energy target speedcoefficient eatTimer myGroup inDanger dangerTimer target-X target-Y target-ID follower following ticksToFollow hasFollowed]
...
to move-zombies[State]
  ...
  if State = "Step4"[
    ask zombies [
      ...
      if(target != nobody and inDanger != 1 and following != 1) [
        face target
        ask zombies in-radius vision-radius [
          if (target = nobody and following != 1) [
      ...
      ;
      let cor min-one-of corpses in-radius vision-radius [distance myself]
      ifelse(inDanger = 0 and (energy = min ([energy] of zombies in-radius vision-radius)) and cor != nobody and following != 1) [
        face cor
      ][
        if(following != 1)[
        right random 45
      ...
      ask one-of zombies in-radius vision-radius[
      ...
      if(follower = nobody)[
        set hasFollowed ticksToFollow + 0
        set ticksToFollow ticksToFollow - ticksToFollow
      ]
      if(following = 1 and follower != 0 and follower != nobody)[
        face follower
        set hasFollowed hasFollowed + 1

      ]

      if(hasFollowed >= ticksToFollow and following != 0)[
        set following 0
        set hasFollowed hasFollowed - hasFollowed
...
to alert
  ...
  if((dangerTimer > 0) and (dangerTimer <= maxDangerTimer))[ ;låter Zombies jaga i grupp
    let vote 0
    let amount count zombies in-radius vision-radius
    if voting [
      ask zombies in-radius vision-radius[
        if(target = nobody)[
          set vote vote + 1
          if Show-Zombie-comms [set pcolor pink]
        ]
        if(target != nobody)[
          set vote vote + 0
          if Show-Zombie-comms [set pcolor red]
        ]
      ]
    ]
...
to eat-corpse
  ask zombies [
    if((energy = min ([energy] of zombies in-radius vision-radius)))[  ; om zombies har en energinivå under 90 kan den äta
